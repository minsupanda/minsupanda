...
void Print(std::vector<int> iList) // 똑같이 매개변수 가져오고
{
...
std::vector <int>::iterator Search(std::vector<int>* iList, int Data)
{
	...
	//	iList[i];
	//}
	// 얘는 vector에서는 가능한 것이고 list에서는 안되는 것 (인덱스로 접근하는 것이기 때문)
...
{
	std::vector<int>::iterator iter = std::find(iList.begin(), iList.end(), Data);
...
void AddData(std::vector<int>* iList, int Data, int Add) // 내가 원하는 위치에 데이터 추가
{
...
void ModifyData(std::vector<int>* iList, int Data, int ModifyData) // 데이터 수정
{
	// 추가와 거의 비슷
	std::vector<int>::iterator iter = Search(iList, Data);
	if (iter != iList->end())
...
void DeleteData(std::vector<int>* iList, int Data)
{
	std::vector<int>::iterator iter = Search(iList, Data);
	if (iter != iList->end())
